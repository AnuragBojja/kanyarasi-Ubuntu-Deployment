...
def captain_register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            user = form.save()  # Get user from form.save() method
            messages.success(request, 'Registration successful')  # Add a success message
            UserProfile.objects.create(user=user, role='captain')  # Create a user profile
            return redirect('captain_login')  # Redirect to captain dashboard
    else:
...
def view_menu(request):
    if request.user.is_authenticated:
        try:
            # Attempt to fetch the Master profile for the logged-in user
            master_profile = Master.objects.get(user=request.user)
            # Fetch menu items related to this Master
            menu_items = MenuItem.objects.filter(master=master_profile)
        except ObjectDoesNotExist:
            # If no Master profile is found for the user, set menu_items to empty
            menu_items = []
    else:
        # If user is not authenticated, set menu_items to empty
        menu_items = []
...
# Assume you have a model for Order and MenuItem
...
@login_required
def order_status(request):
    # Assuming 'delivered' is a status value in your Order model
...
def delete_order(request, order_id):
    # Protect against deleting other users' orders
